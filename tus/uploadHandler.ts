...
import type { R2UploadedPart, DurableObjectState } from '@cloudflare/workers-types'
import type { Context } from 'hono'
import { noopDigester, sha256Digester } from './digest.ts'
import { parseChecksum, parseUploadMetadata } from './parse.ts'
import type { Bindings } from './util.ts'
import {
...
function optionsHandler(c: Context): Response {
  ...
  return c.newResponse(null, 204, {
    ...
    'Access-Control-Allow-Headers': 'Content-Type, Authorization, Content-Length, X-Checksum-SHA256, tus-resumable, tus-version, tus-max-size, tus-extension, tus-checksum-sha256, upload-metadata, upload-length, upload-offset',
...
export class UploadHandler {
  ...
  constructor(state: DurableObjectState, env: Bindings) {
    ...
    this.router.options('/files/:bucket', optionsHandler)
    this.router.post('/files/:bucket', this.exclusive(this.create))
    this.router.options('/files/:bucket/:id{.+}', optionsHandler)
    this.router.patch('/files/:bucket/:id{.+}', this.exclusive(this.patch))
    this.router.get('/files/:bucket/:id{.+}', this.exclusive(this.head))
